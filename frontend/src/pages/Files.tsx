...
const getFileIcon = (name: string, isDir: boolean) => {
  if (isDir) return <Folder size={18} className="text-indigo-400" />;
  ...
  return <FileIcon size={18} className="text-neutral-500" />;
};
...
const getFileTagColor = (name: string) => {
  ...
  if (['md'].includes(ext)) return 'bg-neutral-500/10 text-neutral-400 border-neutral-500/20';
      ...
      <div className="max-w-6xl w-full mx-auto flex flex-col h-full">
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6 shrink-0">
          <div>
            ...
            <p className="text-neutral-600 dark:text-neutral-400 flex items-center gap-2 font-mono text-sm">
              <span className="bg-neutral-100 dark:bg-white/5 border border-neutral-200 dark:border-white/10 px-3 py-1 rounded-lg">
                {directory || 'Loading...'}
              </span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 text-sm">
              <span className="flex items-center gap-1.5 bg-indigo-500/10 text-indigo-500 px-3 py-1.5 rounded-lg border border-indigo-500/20 font-medium">
                <Folder size={14} /> {directories.length}
              </span>
              <span className="flex items-center gap-1.5 bg-neutral-100 dark:bg-white/5 text-neutral-600 dark:text-neutral-400 px-3 py-1.5 rounded-lg border border-neutral-200 dark:border-white/10 font-medium">
        ...
        <div className="flex-1 overflow-y-auto pb-10 custom-scrollbar pr-2">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center p-20 bg-white dark:bg-neutral-900/40 rounded-3xl border border-neutral-200 dark:border-neutral-800/60 border-dashed mt-10 shadow-sm dark:shadow-none transition-colors">
              <HardDrive size={48} className="text-neutral-400 dark:text-neutral-600 mb-4" />
              <h2 className="text-xl font-medium text-neutral-800 dark:text-neutral-300 mb-2">{searchTerm ? 'No matching files' : 'Directory is empty'}</h2>
              <p className="text-neutral-500 text-center max-w-md">{searchTerm ? 'Try adjusting your search query.' : 'This directory doesn\'t contain any visible files or folders.'}</p>
            </div>
          ) : (
                ...
                {sortedFiles.map((f, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      key={f.name}
                      className="flex items-center gap-4 px-5 py-3.5 hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors cursor-default border-b border-neutral-100 dark:border-neutral-800/50 last:border-0 group"
                    >
                      <div className="w-9 h-9 rounded-xl bg-neutral-50 dark:bg-white/5 border border-neutral-100 dark:border-white/10 flex items-center justify-center shrink-0 group-hover:border-brand/30 transition-colors">
                        {getFileIcon(f.name, f.is_dir)}
                      </div>
                      <span className={`flex-1 truncate font-medium text-sm ${f.is_dir ? 'text-neutral-900 dark:text-white' : 'text-neutral-600 dark:text-neutral-300'}`}>
                        {f.name}
                      </span>
                      {!f.is_dir && tagColor && (
                      ...
                      {f.is_dir && (
                        <ChevronRight size={16} className="text-neutral-300 dark:text-neutral-600 group-hover:text-brand transition-colors" />
                      )}
                    </motion.div>
                  );
                })}
