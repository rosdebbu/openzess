  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-transparent p-10 overflow-hidden">
      <div className="max-w-6xl w-full mx-auto flex flex-col h-full">
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6 shrink-0 border-b border-neutral-200 dark:border-neutral-800 pb-6">
          <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-neutral-900 dark:text-neutral-200 pl-11 pr-4 py-2.5 rounded-xl w-56 focus:outline-none focus:border-brand/40 shadow-sm dark:shadow-none transition-colors text-sm font-medium"
              />
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto pb-10 custom-scrollbar pr-2">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center p-20 bg-white dark:bg-neutral-900/40 rounded-3xl border border-neutral-200 dark:border-neutral-800/60 border-dashed mt-10">
              <Wrench size={48} className="text-neutral-400 dark:text-neutral-600 mb-4" />
              <h2 className="text-xl font-medium text-neutral-800 dark:text-neutral-300 mb-2">{searchTerm ? 'No matching tools' : 'No tools available'}</h2>
              <p className="text-neutral-500 text-center max-w-md">{searchTerm ? 'Try adjusting your search query.' : 'The agent hasn\'t loaded any native tools yet.'}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredTools.map((t, i) => (
                <motion.div
                  ...
                  key={i}
                  className="bg-white dark:bg-neutral-900/60 border border-neutral-200 dark:border-neutral-800 rounded-3xl overflow-hidden flex flex-col shadow-sm dark:shadow-none hover:border-brand/40 hover:-translate-y-0.5 transition-all group"
                >
                  {/* Gradient Banner */}
                  ...
                  <div className="p-5 flex flex-col flex-1 -mt-5 relative">
                    {/* Floating Icon */}
                    <div className={`w-10 h-10 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 flex items-center justify-center mb-4 shadow-lg ${getToolIconColor(i)} group-hover:scale-105 transition-transform`}>
                      {getToolIcon(t.name)}
                    </div>
                    
                    <h3 className="font-bold text-neutral-900 dark:text-white mb-2 font-mono text-sm leading-tight">{t.name}</h3>
                    <p className="text-neutral-500 dark:text-neutral-400 text-sm leading-relaxed flex-1">{t.description}</p>
                    
                    <div className="mt-4 pt-3 border-t border-neutral-100 dark:border-neutral-800/50">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-neutral-400 bg-neutral-50 dark:bg-white/5 px-2 py-1 rounded-md border border-neutral-200 dark:border-white/10">Native Tool</span>
                    </div>
                  </div>
                </motion.div>
              ))}
