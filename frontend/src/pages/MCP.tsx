...
const DEFAULT_SERVERS: MCPServerInfo[] = [
  ...
  { 
     ...
     transport: 'stdio',
     command: 'npx',
     args: ['-y', 'mcp-remote', 'https://stitch.googleapis.com/mcp', '--header', `X-Goog-Api-Key: ${import.meta.env?.VITE_STITCH_API_KEY || ''}`]
  },
];
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
           ...
           <div className="flex items-center gap-4">
              <button 
                 ...
                 className="flex items-center gap-2 bg-[#1E1C1C] dark:bg-white text-white dark:text-[#3A3838] px-4 py-2 rounded-xl font-medium text-sm hover:scale-[0.98] transition-transform shadow-sm"
              >
                  <Plus size={16} /> Add Protocol
              </button>
              
              <div className="relative">
                 <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#B8AFA8]" size={18} />
              <input 
                 type="text" 
                 placeholder="Search protocols..." 
                 value={searchTerm}
                 onChange={e => setSearchTerm(e.target.value)}
                 className="pl-10 pr-4 py-2 bg-white dark:bg-surface border border-[#E2DAD2] dark:border-border rounded-xl focus:outline-none focus:border-brand transition-colors shadow-sm w-full md:w-64"
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredServers.map((server, i) => {
             ...
             <motion.div 
               ...
               key={server.id}
               className={`bg-white dark:bg-surface border ${isConnected ? 'border-emerald-500/30 shadow-emerald-500/10' : 'border-[#E2DAD2] dark:border-border hover:border-brand/50 hover:shadow-xl'} p-6 rounded-2xl shadow-sm transition-all group flex flex-col h-full relative overflow-hidden`}
             >
                {isLoading && (
                   <div className="absolute inset-0 bg-white/50 dark:bg-[#1E1C1C]/50 backdrop-blur-[2px] flex items-center justify-center z-10">
                   ...
                   <div className="flex items-center gap-2">
                       {server.isCustom && (
                          <button 
                             ...
                             className="p-1.5 text-[#B8AFA8] hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors"
                       ...
                       {isConnected ? (
                          ...
                          <span className="flex items-center gap-1 text-xs font-medium bg-[#EDE8E2] dark:bg-[#1E1C1C] text-[#B8AFA8] px-2 py-1 rounded-md">
                ...
                {isConnected && serverStats[server.id]?.toolNames && (
                   <div className="mb-6 flex flex-wrap gap-2">
                      {serverStats[server.id].toolNames.length === 0 ? (
                         <span className="text-xs text-[#B8AFA8] italic">No tools exposed</span>
                      ) : (
                         serverStats[server.id].toolNames.map((tName: string) => (
                            <span key={tName} className="flex items-center gap-1 text-[11px] bg-[#EDE8E2] dark:bg-[#2A2828] text-[#3A3838]/80 dark:text-[#E2DAD2]/80 px-2 py-1 rounded-md border border-[#E2DAD2] dark:border-[#3A3838]">
                ...
                <button 
                   ...
                   disabled={isLoading}
                   className={`w-full py-2.5 rounded-xl font-medium text-sm flex items-center justify-center gap-2 transition-colors ${isConnected ? 'bg-[#EDE8E2] dark:bg-[#1E1C1C] text-[#3A3838] dark:text-[#E2DAD2]/80 hover:bg-red-500/10 hover:text-red-500' : 'bg-brand hover:bg-brand-hover text-white shadow-lg shadow-brand/20'}`}
                >
