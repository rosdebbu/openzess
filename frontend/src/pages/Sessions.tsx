    ...
    <div className="flex-1 flex flex-col h-full bg-transparent p-10 overflow-hidden">
      <div className="max-w-6xl w-full mx-auto flex flex-col h-full">
        <header className="mb-8 flex items-end justify-between shrink-0">
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-3 text-[#3A3838] dark:text-[#E2DAD2] mb-2">
              <History className="text-brand" /> Past Chats
            </h1>
            <p className="text-[#3A3838]/80 dark:text-[#B8AFA8]">Resume your previous discussions and memory traces.</p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {sessions.map((session, i) => (
                <motion.div
                  ...
                  className="group bg-white dark:bg-[#1E1C1C]/50 hover:bg-[#EDE8E2] dark:hover:bg-[#2A2828]/80 border border-[#E2DAD2] dark:border-[#3A3838] hover:border-brand/30 transition-all cursor-pointer rounded-2xl p-6 relative overflow-hidden flex flex-col shadow-sm dark:shadow-none hover:shadow-xl hover:shadow-brand/5"
                >
                  ...
                  <h3 className="font-medium text-[#3A3838] dark:text-[#E2DAD2] mb-3 line-clamp-2 leading-relaxed h-[48px] relative z-10 group-hover:text-[#3A3838] dark:group-hover:text-white transition-colors pr-8">
                    {session.title || "New Conversation"}
                  </h3>
                  
                  <button 
                     onClick={(e) => handleDelete(e, session.id)} 
                     className="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 p-2 text-[#B8AFA8] hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition-all"
                     title="Delete Session"
                  >
                     <Trash2 size={16} />
                  </button>
                  
                  <div className="mt-auto flex items-center justify-between text-[#B8AFA8] text-xs font-medium pt-4 border-t border-[#E2DAD2] dark:border-[#3A3838]/50 relative z-10">
